...
public class DeployEclipse {
  ...
  /*      String path = this.getClass().getResource("").getPath();
    ...
    private String[] unDeploy(String[] arr) {
        ...
        String  [] undeployRoles= DelpoyConfig.getInstance(2).getValue("undeployRole").split(",");
    ...
    private void copyFile(List<ContentBean> ContentBeans){
        for (ContentBean cb : ContentBeans) {
            String resPath = DelpoyConfig.getInstance(2).getValue("appPath")+"\\"
                    + DelpoyConfig.getInstance(2).getValue("projectName");
            String deployPath = DelpoyConfig.getInstance(2).getValue("deployPath")+"\\"
                    + DelpoyConfig.getInstance(2).getValue("projectName");
            FileHelper fileHelper = new FileHelper();
...
//		System.out.println("M /branches/booking2.977/src/common/CommonMethod.java".matches(".*src/common.*"));
        DeployEclipse ms = new DeployEclipse();
